...
import os, csv, gc, itertools, shutil, torch, numpy as np, pandas as pd, sys
...
data_dir = 'F:/Paper2Data/testData4'
DATASETS = ['Patent', 'celegans', 'human', 'Article']

def ensure_mixed_graphs():
       
                
       
    for dataset in DATASETS:
        real_dir  = os.path.join(data_dir, dataset, 'ProteinGraph', 'package_bin')
        pred_dir  = os.path.join(data_dir, dataset, 'pred_graphs', 'package_bin')
        mixed_dir = os.path.join(data_dir, dataset, 'mixed_graphs', 'package_bin')
        os.makedirs(mixed_dir, exist_ok=True)
        copied, skipped = 0, 0

        def _copy_if_needed(src):
            nonlocal copied, skipped
            dst = os.path.join(mixed_dir, os.path.basename(src))
        ...
        if os.path.isdir(real_dir):
            for fn in os.listdir(real_dir):
                if fn.endswith('.bin'):
                    _copy_if_needed(os.path.join(real_dir, fn))
        else:
            print(f"[mixed_graphs][WARN] 真实结构目录不存在: {real_dir}")

        if os.path.isdir(pred_dir):
            for fn in os.listdir(pred_dir):
                if fn.startswith('ESM_') and fn.endswith('.bin'):
                    _copy_if_needed(os.path.join(pred_dir, fn))
        else:
            print(f"[mixed_graphs][WARN] 预测结构目录不存在: {pred_dir}")

        total = len([f for f in os.listdir(mixed_dir) if f.endswith('.bin')])
        print(f"[mixed_graphs] {dataset}: 新复制{copied} 跳过{skipped} 现有{total}个.bin")
...
def run_training():
    ...
    base_out = 'F:/LINUX_code/solve_OOD_to24G_LocalFinal/output/batch_results_OOD_Exp1_noAttn'
    ...
    for dataset, split, seed in schedule:
        ...
        s2p_json   = os.path.join(data_dir, dataset, 'sequence_to_pdb_mixed.json')
        cg_dir     = os.path.join(data_dir, dataset, 'CompoundGraph')
        pg_dir     = os.path.join(data_dir, dataset, 'mixed_graphs', 'package_bin')
