...
from unified_dataset_v2woPG import UnifiedDtiDatasetV2, unified_collate_fn_v2
...
def run_training():
    datasets = ['Patent','celegans','Article','human']
    ood_splits = ['random','cluster', 'cold_smiles', 'cold_protein', 'cold_both']
    random_split = []
    all_splits = ood_splits + random_split
    
    phase1 = list(itertools.product(datasets, ood_splits, []))
    phase2 = list(itertools.product(datasets, random_split, []))
    phase3 = list(itertools.product(datasets, all_splits, [42,1234, 2025, 12345, 666]))
    ...
    base_out = 'E:/LINUX_code/solve_OOD_to24G_LocalFinal/output/batch_results_OOD_noAttn_woPG'
