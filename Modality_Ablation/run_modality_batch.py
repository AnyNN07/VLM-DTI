...
from model_modality import UniQCM2Net_Modality
...
def run_training():
    ...
    base_out = 'E:/LINUX_code/solve_OOD_to24G_LocalFinal/output/Modality_Ablation_Results'
    os.makedirs(base_out, exist_ok=True)
    out_csv = os.path.join(base_out, 'results_modality_ablation.csv')

    completed = set()
    if os.path.exists(out_csv):
        with open(out_csv, 'r', newline='', encoding='utf-8') as f:
            for row in csv.DictReader(f):
                completed.add((row['Modality'], row['Dataset'], row['Split'], int(row['Seed'])))
    else:
        with open(out_csv, 'w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow(['Modality','Dataset','Split','Seed','BestEpoch','AUC_Test','PRAUC_Test',
    ...
    for modality in modalities:
        ...
        for dataset, split, seed in schedule:
            if (modality, dataset, split, seed) in completed:
                print(f"SKIP {modality} | {dataset}/{split}/seed={seed} (done)")
                continue

            print(f"\n=> {modality} | {dataset}/{split}/seed={seed}", flush=True)
            set_all_seeds(seed)
            ...
            load_llm = modality in ['llm_only', 'full']
            train_ds = UnifiedDtiDatasetV2(tr_df, smiles_npy, p_lm_pkl, d_lm_pkl, s2p_json, cg_dir, pg_dir, load_llm=load_llm)
            val_ds   = UnifiedDtiDatasetV2(vl_df, smiles_npy, p_lm_pkl, d_lm_pkl, s2p_json, cg_dir, pg_dir, load_llm=load_llm)
            test_ds  = UnifiedDtiDatasetV2(ts_df, smiles_npy, p_lm_pkl, d_lm_pkl, s2p_json, cg_dir, pg_dir, load_llm=load_llm)
            ...
            model = UniQCM2Net_Modality(gnn_hidden=h, input_modality=modality).to(device)
            ...
            best_metrics = None
            model_dir = os.path.join(base_out, f'models/{modality}/{dataset}/{split}')
            ...
            for ep in range(1, max_epochs + 1):
                ...
                pbar = tqdm(tl, desc=f"E{ep:2d}/{max_epochs}", leave=False)
                ...
                print(f"E{ep:2d} Loss={ep_loss/len(tl):.4f} ValAUC={val_auc:.4f}")
            ...
            with open(out_csv, 'a', newline='', encoding='utf-8') as f:
                csv.writer(f).writerow([modality, dataset, split, seed, best_ep,
            ...
            print(f"DONE {modality} | {dataset}/{split}/seed={seed}: Best Test AUC={best_metrics['auc']:.4f}")
