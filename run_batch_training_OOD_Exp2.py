...
default_project_dir = r'E:\LINUX_code\solve_OOD_to24G_LocalFinal'
project_dir = (
    script_dir
    if os.path.exists(os.path.join(script_dir, 'unified_dataset_v2.py'))
    else default_project_dir
)
sys.path.insert(0, project_dir)
sys.path.insert(0, os.path.join(project_dir, 'VLMNET'))
...
def test_epoch(model, dataloader):
    ...
    with torch.inference_mode():
        for bg_drug, bg_prot, d_feat, p_feat, labels, d_lens, p_lens in dataloader:
            ...
            y_label.extend(labels.detach().cpu().numpy())
...
def run_training():
    datasets = ['human','Article','Patent', 'celegans']
    ood_splits = []
    random_split = ['random']
    ...
    phase3 = list(itertools.product(datasets, all_splits, [666, 12345,2025,1234]))
    ...
    base_out = os.path.join(project_dir, 'output', 'batch_results_OOD_Exp2_noAttn')
    ...
    for dataset, split, seed in schedule:
        ...
        pg_dir     = os.path.join(data_dir, dataset, 'pred_graphs', 'package_bin')
        ...
        tl = DataLoader(
            ...
            batch_size=bs,
            shuffle=True,
            collate_fn=unified_collate_fn_v2,
